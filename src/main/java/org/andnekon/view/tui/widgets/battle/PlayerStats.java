...
public class PlayerStats implements Widget {
    ...
    public void draw(Screen screen) {
        String[] recourcesString = new String[]{"hull", String.valueOf(manager.getPlayer().getHp()),
            "shield", String.valueOf(manager.getPlayer().getDefense()),
            "energy", String.valueOf(manager.getPlayer().getEnergy())};
        resources = createWidget(recourcesString);
        resources.draw(screen);
    }

    private MultiLine createWidget(String[] lines) {
        return new MultiLine(topLeft.getColumn(), topLeft.getRow(),
